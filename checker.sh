...
function valgrindTest() {
    ...
    #valgrind --leak-check=full --error-exitcode=1 ./tema1 1> ./output/valgrindCopy
    if [ "$?" -ne 0 ]
        then
...
function testInput() {
    ...
    ./tema1
    cat tema1.out > ./output/test$1.out
    if [ "$?" -eq 139 ]
        then
    ...
    #Teste/Output/test$1.out tema1.out > ./output/test$1.out
    if [ "$?" -ne 0 ]
        then
